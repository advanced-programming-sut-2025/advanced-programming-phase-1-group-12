...
public class ToolsController {
    ...
    public Result updateToolsCheck() {
        if(App.getCurrentPlayerLazy().getCurrentTool() == null) {
            return new Result(false, "You don't have any tools in your backpack");
        }
        if (!checkIsInSmithing()) {
            return new Result(false, "You need to be in the Blacksmith's shop to upgrade tools");
        }

        if (!App.getCurrentPlayerLazy().getCurrentTool().canUpgrade()) {
        ...
        Result upgradeResult = App.getCurrentPlayerLazy().getCurrentTool().
            upgrade(App.getCurrentPlayerLazy().getCurrentTool().getLevel() + 1);

        if (upgradeResult.isSuccessful()) {
            return new Result(true, "Successfully upgraded your " + App.getCurrentPlayerLazy().getCurrentTool()
                .getToolType().getName() + " to " + getLevelName(currentTool.getLevel()));
        } else {
