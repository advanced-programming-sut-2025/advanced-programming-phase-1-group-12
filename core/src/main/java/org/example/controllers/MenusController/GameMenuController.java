...
//            case STONE -> "\u001B[103m";
    ...
    public Result trashItem(String name, String amount) {
        Player player = App.getCurrentGame().getCurrentPlayer();
        BackPack backPack = player.getBackPack();
        org.example.models.ToolsPackage.Tools trashCan = null;
        for (Item item : backPack.getItems().keySet()) {
            if (item instanceof org.example.models.ToolsPackage.Tools) {
                org.example.models.ToolsPackage.Tools tool = (org.example.models.ToolsPackage.Tools) item;
                if (tool.isTrashCan()) {
                    trashCan = tool;
                    break;
                }
    ...
    public Result Thor(int x, int y, Stage stage) {
        Location findLocation = App.getCurrentGame().getMainMap().findLocation(x, y);
        findLocation.setTypeOfTile(TypeOfTile.BURNED_GROUND);
        // Black overlay
