    ...
    public void render(SpriteBatch batch, int offsetX, int offsetY) {
        ...
        for (Location loc : gameMap.getTilesOfMap()) {
            ...
            if (loc.getTypeOfTile() == TypeOfTile.PLANT) {
                Plant plant = (Plant) loc.getObjectInTile();
                int waterCount = plant.getDayPast();
                String waterText = ":" + waterCount;
//                String waterText = "💧".repeat(Math.max(0, waterCount)) + waterCount;
                font.draw(batch, waterText, offsetX + loc.getxAxis() * tileSize,
                    offsetY + (loc.getyAxis() * tileSize) + tileSize);
            }
        ...
        for (Location loc : App.getCurrentGame().getMainMap().getTilesOfMap()) {
            ...
            if (loc.getTypeOfTile() == TypeOfTile.PLANT) {
                ...
                Stone stone = (Stone) loc.getObjectInTile();
                batch.draw(stone.getMineralTypes().getTexture(), drawX, drawY, 50, 50);
