...
public class GameMenu extends InputAdapter implements Screen {
    ...
    private List<CrowAnimation> crowAnimations = new ArrayList<>();
    private float timeSinceEnergyFinished = 0f;
    private static final float ENERGY_DISPLAY_TIME = 5f;
    ...
    private Map<Ability, ProgressBar> abilityProgressBars = new HashMap<>();
    private Label fishingAbilityLabel, miningAbilityLabel, farmingAbilityLabel, foragingAbilityLabel;
    private ProgressBar fishingProgressBar;
    private ProgressBar miningProgressBar;
    private ProgressBar farmingProgressBar;
    private ProgressBar foragingProgressBar;
    ...
    public void show() {
        ...
        ImageButton BackPackButton = new ImageButton(new TextureRegionDrawable(new TextureRegion(ToolAssetsManager.toolAssetsManager().chooseBackPack())));
        BackPackButton.setPosition(Gdx.graphics.getWidth() - 60, 10);
        ...
        float abilityBarHeight = 20f;
        float abilityYOffset = stage.getHeight() - 120f; // Adjust yOffset for spacing
        float abilitySpacing = 30f;
        float abilityLeftMargin = 20f;

        // Fishing ability bar
        fishingAbilityLabel = new Label("Fishing", skin);
        fishingProgressBar = new ProgressBar(0, 100, 1, false, skin.get("default-horizontal", ProgressBar.ProgressBarStyle.class));
        fishingProgressBar.setValue(currentPlayer.getAbilityByName("Fishing").getAmount());
        fishingProgressBar.setSize(200, abilityBarHeight);
        fishingProgressBar.setPosition(abilityLeftMargin, abilityYOffset);
        stage.addActor(fishingAbilityLabel);
        stage.addActor(fishingProgressBar);
        abilityProgressBars.put(currentPlayer.getAbilityByName("Fishing"), fishingProgressBar);
        fishingAbilityLabel.setPosition(abilityLeftMargin, abilityYOffset + 25f); // Positioning label below the bar

        // Mining ability bar
        miningAbilityLabel = new Label("Mining", skin);
        miningProgressBar = new ProgressBar(0, 100, 1, false, skin.get("default-horizontal", ProgressBar.ProgressBarStyle.class));
        miningProgressBar.setValue(currentPlayer.getAbilityByName("Mining").getAmount());
        miningProgressBar.setSize(200, abilityBarHeight);
        miningProgressBar.setPosition(abilityLeftMargin, abilityYOffset - abilitySpacing);
        stage.addActor(miningAbilityLabel);
        stage.addActor(miningProgressBar);
        abilityProgressBars.put(currentPlayer.getAbilityByName("Mining"), miningProgressBar);
        miningAbilityLabel.setPosition(abilityLeftMargin, abilityYOffset - abilitySpacing + 25f);

        // Farming ability bar
        farmingAbilityLabel = new Label("Farming", skin);
        farmingProgressBar = new ProgressBar(0, 100, 1, false, skin.get("default-horizontal", ProgressBar.ProgressBarStyle.class));
        farmingProgressBar.setValue(currentPlayer.getAbilityByName("Farming").getAmount());
        farmingProgressBar.setSize(200, abilityBarHeight);
        farmingProgressBar.setPosition(abilityLeftMargin, abilityYOffset - 2 * abilitySpacing);
        stage.addActor(farmingAbilityLabel);
        stage.addActor(farmingProgressBar);
        abilityProgressBars.put(currentPlayer.getAbilityByName("Farming"), farmingProgressBar);
        farmingAbilityLabel.setPosition(abilityLeftMargin, abilityYOffset - 2 * abilitySpacing + 25f);

        // Foraging ability bar
        foragingAbilityLabel = new Label("Foraging", skin);
        foragingProgressBar = new ProgressBar(0, 100, 1, false, skin.get("default-horizontal", ProgressBar.ProgressBarStyle.class));
        foragingProgressBar.setValue(currentPlayer.getAbilityByName("Foraging").getAmount());
        foragingProgressBar.setSize(200, abilityBarHeight);
        foragingProgressBar.setPosition(abilityLeftMargin, abilityYOffset - 3 * abilitySpacing);
        stage.addActor(foragingAbilityLabel);
        stage.addActor(foragingProgressBar);
        abilityProgressBars.put(currentPlayer.getAbilityByName("Foraging"), foragingProgressBar);
        foragingAbilityLabel.setPosition(abilityLeftMargin, abilityYOffset - 3 * abilitySpacing + 25f);
    ...
    public void render(float delta) {
        ...
        updateAbilityProgressBar(currentPlayer.getAbilityByName("Fishing"), fishingAbilityLabel, fishingProgressBar);
        updateAbilityProgressBar(currentPlayer.getAbilityByName("Mining"), miningAbilityLabel, miningProgressBar);
        updateAbilityProgressBar(currentPlayer.getAbilityByName("Farming"), farmingAbilityLabel, farmingProgressBar);
        updateAbilityProgressBar(currentPlayer.getAbilityByName("Foraging"), foragingAbilityLabel, foragingProgressBar);
        ...
        for (Player p : App.getCurrentGame().getPlayers()) {
            ...
            if (bar != null) {
                ...
                if (p.getEnergy() <= 0 && App.getCurrentPlayerLazy().equals(p)) {
                    timeSinceEnergyFinished += delta;
                    if (timeSinceEnergyFinished >= ENERGY_DISPLAY_TIME) {
                        controller.nextTurn();
                        break;
                    }
                }
        ...
        Iterator<CrowAnimation> iterator = crowAnimations.iterator();
        ...
        for (CrowAnimation crow : crowAnimations) {
            batch.draw(crow.currentFrame, crow.x, crow.y, 88, 88);
        }

        int hour = App.getCurrentGame().getDate().getHour();
        if (hour == 11 && isAttacking) {
            attackingCrow();
            isAttacking = false;
        } else if (hour == 12) isAttacking = true;
    ...
    private void updateAbilityProgressBar(Ability ability, Label label, ProgressBar progressBar) {
        if (progressBar == null) {
            progressBar = new ProgressBar(0, 100, 1, false, skin.get("default-horizontal", ProgressBar.ProgressBarStyle.class));
            stage.addActor(progressBar);
        }

        progressBar.setValue(ability.getAmount());
        label.setText(ability.getName() + " - Level: " + ability.getLevel());

        label.setPosition(progressBar.getX() + progressBar.getWidth() / 2 - label.getWidth() / 2, progressBar.getY() + progressBar.getHeight() + 5f);
    }
    ...
    public boolean keyDown(int keycode) {
        ...
        if (keycode == Input.Keys.U) {
            Result result = toolsController.updateToolsCheck();
    ...
    public void attackingCrow() {
        if (App.getCurrentPlayerLazy().getOwnedFarm().getPlantOfFarm().size() < 3) return;
        ...
        if (randomInt < 50 && !plants.isEmpty()) {
            List<Plant> destroyablePlants = new ArrayList<>(plants);
            Collections.shuffle(destroyablePlants);

            for (int i = 0; i < 2; i++) {
                Plant plant = destroyablePlants.get(i);
                Location location = plant.getLocation();
                if (location.getObjectInTile() instanceof ArtisanItem) break;

                location.setObjectInTile(null);
                location.setTypeOfTile(TypeOfTile.PLOUGHED_LAND);
                plants.remove(plant);

                spawnCrowAtLocation(location);
            }
        }
    }

    private void spawnCrowAtLocation(Location location) {
        float crowX = location.getxAxis() * 100f;
        float crowY = location.getyAxis() * 100f + 30f;
        Animation<TextureRegion> crowAnim = AnimalAssetsManager.getCrowAnimation();
        CrowAnimation crowAnimation = new CrowAnimation(crowX, crowY, crowAnim);

        crowAnimations.add(crowAnimation);
    }
